...
function findTopic(req, res, next) {
  if (req.params.id) {
...
//  }
  ...
  var id = req.query.forum || (req.topic && req.topic.forum && req.topic.forum._id.toString())
  if (id) {
...
function onlyForumAdmins(req, res, next) {
  if (!req.user) _handleUnauthorizedError(new Error('Unauthorized.'), req, res);

  if (config.multiForum) {
    if (req.forum.isAdmin(req.user)) return next();
    return _handleUnauthorizedError(new Error('Unauthorized.'), req, res);
  }
...
function canVoteAndComment(req, res, next) {
  if (!config.multiForum || req.user.staff || !req.forum.private) {
    return next();
  }
  ...
  if (req.forum && (req.forum.isAdmin(req.user) || req.forum.hasRole('admin', req.user) || req.forum.hasRole('collaborator', req.user))) {
    return next();
  } else {
    return _handleUnauthorizedError(new Error('Unauthorized.'), req, res);
  }
...
app.post('/topic/:id/comment', restrict, findTopic, findForum, canVoteAndComment, function (req, res) {
...
app.post('/topic/:id/vote', restrict, findTopic, findForum, canVoteAndComment, function (req, res) {
...
function _handleUnauthorizedError (err, req, res) {
  log("Error found: %s", err);
  var error = err;
  if (err.errors && err.errors.text) error = err.errors.text;
  if (error.type) error = error.type;
