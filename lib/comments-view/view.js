...
export default class CommentsView extends FormView {
  ...
  constructor (type, item, reference) {
    super(template, { type: type, item: item, reference: reference });
    this.type = type;
    this.item = item;
  ...
  initializeComments () {
    ...
    commentStore
      .findAll({type: this.type, id: this.item.id, count: true, sort: this.sort, exclude_user: user.id || null})
      .then(count => {
        this.count = count;
        this.fetch();
      })
  ...
  initializeMyComments () {
    if (user.id) {
      commentStore
        .myComments({type: this.type, id: this.item.id})
        .then(comments => {
